...
from utils.fileIO import load, load_effic_file, loadExp, write_to_xls
...
def setupMenus(CS_exists):
    ...
    options_menu.add_separator()
    options_menu.add_command(label="Export Spectrum", command=lambda: write_to_xls(satelite_var.get(), energy_offset.get(), yoffset.get(), _residues_graph))
    options_menu.add_separator()
...
def setupExpPlot(f, load, element_name):
    ...
    _residues_graph = residues_graph
